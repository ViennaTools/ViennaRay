...
template <size_t Dim>
void PointNeighborhood<NumericType, D>::init(
...
template <typename NumericType, int D>
void PointNeighborhood<NumericType, D>::createNeighborhood(
    const std::vector<Vec3D<NumericType>> &points,
    const std::vector<unsigned int> &side1,
    const std::vector<unsigned int> &side2, const Vec3D<NumericType> &min,
    const Vec3D<NumericType> &max, const int &dirIdx,
    const std::vector<int> &dirs, const NumericType &pivot) {
  ...
  if (side1.size() > 1) {
    ...
    createNeighborhood(points, s1r1set, s1r2set, min, newS1Max, newDirIdx, dirs,
                       newPivot);
  }
  if (side2.size() > 1) {
    auto newS2Min = min;
    newS2Min[dirs[dirIdx]] = pivot; // old diridx and old pivot!
    createNeighborhood(points, s2r1set, s2r2set, newS2Min, max, newDirIdx, dirs,
                       newPivot);
  }
}
...
template <typename NumericType, int D>
template <size_t Dim>
bool PointNeighborhood<NumericType, D>::checkDistance(
    const VectorType<NumericType, Dim> &p1,
...
SOURCE_INSTANTIATE_TEMPLATE_CLASS_NT_D(PointNeighborhood)
