...
Boundary<NumericType, D>::Boundary(RTCDevice &device,
                                   BoundingBoxType const &boundingBox,
...
template <typename NumericType, int D>
void Boundary<NumericType, D>::processHit(RTCRayHit &rayHit,
                                          bool &reflect) const {
  const auto primID = rayHit.hit.primID;

  // Ray hits backside of boundary
  const auto rayDir =
      Vec3D<NumericType>{rayHit.ray.dir_x, rayHit.ray.dir_y, rayHit.ray.dir_z};
...
template <typename NumericType, int D>
void Boundary<NumericType, D>::releaseGeometry() {
  // Attention:
...
template <typename NumericType, int D>
Vec3D<rayInternal::rtcNumericType>
Boundary<NumericType, D>::getNewOrigin(const RTCRay &ray) {
...
template <typename NumericType, int D>
void Boundary<NumericType, D>::initBoundary(RTCDevice &pDevice) {
...
template <typename NumericType, int D>
Vec3D<Vec3D<NumericType>>
Boundary<NumericType, D>::getTriangleCoords(const size_t primID) const {
...
template <typename NumericType, int D>
void Boundary<NumericType, D>::reflectRay(RTCRayHit &rayHit) {
  auto dir = *reinterpret_cast<Vec3D<rayInternal::rtcNumericType> *>(
      &rayHit.ray.dir_x);
  auto normal =
      *reinterpret_cast<Vec3D<rayInternal::rtcNumericType> *>(&rayHit.hit.Ng_x);
  Normalize(dir);
  Normalize(normal);
...
SOURCE_INSTANTIATE_TEMPLATE_CLASS_NT_D(Boundary)
