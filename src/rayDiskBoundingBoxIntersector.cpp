...
NumericType DiskBoundingBoxXYIntersector<NumericType>::areaInside(
    const PrimType &disk, const NormalType &diskNormal) {
...
template <class NumericType>
NumericType DiskBoundingBoxXYIntersector<NumericType>::computeAreaOutside(
    const PrimType &disk, const Vec3D<NumericType> &dnormal,
  ...
  for (size_t idx = 0; idx < distObjs.size(); ++idx) {
    ...
    if (-radius < d1 && d1 < radius && -radius < d2 && d2 < radius) {
      ...
      auto intersectionpoint = intersectionPointPlaneAndXY(
          bbp1point, bbp1normal, dpoint[0], dpoint[1]);
      auto iDir1 = getIntersectionVector(bbp1normal, bbp2normal);
      auto iDir2 = Inv(iDir1);

      if (!sameDirection(iDir1, dnormal)) {
        iDir1 = iDir2;
      }
...
template <class NumericType>
void DiskBoundingBoxXYIntersector<NumericType>::fillBBoxTransforms() {
...
template <class NumericType>
std::array<typename DiskBoundingBoxXYIntersector<NumericType>::DistanceObject,
           4>
DiskBoundingBoxXYIntersector<NumericType>::computeClosestApproach(
    const PrimType &disk, const Vec3D<NumericType> &dnormal) {
...
template <class NumericType>
NumericType DiskBoundingBoxXYIntersector<NumericType>::computeClosestApproach(
    const PrimType &disk_, const Vec3D<NumericType> &diskNormal, bool swapXY,
    bool reflectXcoords) {
  assertInvariants();
...
template <class NumericType>
Vec3D<NumericType>
DiskBoundingBoxXYIntersector<NumericType>::intersectionPointPlaneAndXY(
    const Vec3D<NumericType> &point, const Vec3D<NumericType> &normal,
    NumericType xx, NumericType yy) {
...
template <class NumericType>
Vec3D<NumericType>
DiskBoundingBoxXYIntersector<NumericType>::getIntersectionVector(
    const Vec3D<NumericType> &n1, const Vec3D<NumericType> &n2) {
...
template <class NumericType>
bool DiskBoundingBoxXYIntersector<NumericType>::sameDirection(
    const Vec3D<NumericType> &v1, const Vec3D<NumericType> &v2) {
  return DotProduct(v1, v2) >= 0;
}

template <class NumericType>
Vec3D<NumericType>
DiskBoundingBoxXYIntersector<NumericType>::getCircPointIntersect(
    Vec3D<NumericType> const &dpoint, Vec3D<NumericType> const &iPoint,
    Vec3D<NumericType> const &iDir, NumericType const &radius,
    NumericType const &d) {
...
template <class NumericType>
void DiskBoundingBoxXYIntersector<NumericType>::assertInvariants() {
  for (auto const &v1 : std::vector<bool>{false, true}) {
    for (auto const &v2 : std::vector<bool>{false, true}) {
      assert(bboxTransforms[v1][v2].low.xx <= bboxTransforms[v1][v2].high.xx &&
             "Assertion");
      assert(bboxTransforms[v1][v2].low.yy <= bboxTransforms[v1][v2].high.yy &&
             "Assertion");
    }
  }
}

SOURCE_INSTANTIATE_TEMPLATE_CLASS_NT(DiskBoundingBoxXYIntersector)
