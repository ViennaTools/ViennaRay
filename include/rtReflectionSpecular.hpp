...
public:
  ...
  static rtPair<rtTriple<NumericType>> use(RTCRay &rayin, RTCHit &hitin) {
    auto normal =
    ...
    assert(rtInternal::IsNormalized(normal) &&
           "rtReflectionSpecular: Surface normal is not normalized");

    auto dirOldInv = rtInternal::Inv(
        rtTriple<NumericType>{rayin.dir_x, rayin.dir_y, rayin.dir_z});
    assert(rtInternal::IsNormalized(dirOldInv) &&
           "rtReflectionSpecular: Surface normal is not normalized");

    // Compute new direction
    auto direction = rtInternal::Diff(
        rtInternal::Scale(2 * rtInternal::DotProduct(normal, dirOldInv),
                          normal),
        dirOldInv);
