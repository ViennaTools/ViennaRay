...
public:
  static bool intersect(RTCRay const &ray, rtQuadruple<NumericType> const &disc,
    ...
    auto const &rayOrigin =
        *reinterpret_cast<rtTriple<NumericType> const *>(&ray.org_x);
    auto const &rayDirection =
        *reinterpret_cast<rtTriple<NumericType> const *>(&ray.dir_x);
    auto const &discOrigin =
        *reinterpret_cast<rtTriple<NumericType> const *>(&disc);
    ...
    // copy ray direction
    auto rayDirectionC = rtTriple<NumericType>{rayDirection[0], rayDirection[1],
                                               rayDirection[2]};
